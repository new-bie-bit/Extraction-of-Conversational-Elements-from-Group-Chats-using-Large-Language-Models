...
import random
data_dir = "./data"
train_file = "check_result4.json"
output_data = pd.read_json(os.path.join(data_dir, train_file))
def check_prompt(x):
	# 提示词，我们交代清楚大模型的角色、目标、注意事项，然后提供背景信息，输出格式就可以了
    prompt = f"""Instruction:
你是一个信息要素提取工作的信息核查人员，下面你需要核验给定的`ChatText`中提取出**客户**的`Infos`中相关信息，参照相关人员的意见，再次从ChatText中核验'Infos'中的信息是否正确，
下面是对话相关信息ChatText:
{x["input"]}
如下是你需要核验的Infos信息和给出的相关意见:
{x['infos_3']}
对于错误的信息，你需要进行更正，并保证Infos的数据格式不变
注意事项：
1. 核验一定要严格,逐条进行核验检查！！！
2. 保持`Infos`的JSON格式不变，没有的信息项也要保留！！！
3. 对于有错误的数据项,修正一定要有理有据，并严格保证数据格式不发生改变!!!
"""
	# 要求的输出格式
...
"""
    total=prompt+infos
    return total
output_data['check_anagin'] = output_data.apply(check_prompt, axis=1)
spark = ChatSparkLLM(
...
def save_result(data):
    with open("./data/check_result4_1.json", "w",encoding='utf-8') as f:
...
for j in range(0, 2):
    ...
    for i in tqdm(range(len(output_data)), desc=f"正在询问第{j}轮"):
        messages = [ChatMessage(
            role="user",
            content=output_data.iloc[i]["check_again"]
        )]
        while True:
            try:
